...
The class definition was by default placed in the `.GlobalEnv` so that it is readily accessible. However, the object only has the default methods (such as `model.apply`) assigned and so doesnt do anything yet. The default methods can be updated using other helper funtions which are described in the following sections.
...
For the example object we will print the inputs in addition to the standard output.
